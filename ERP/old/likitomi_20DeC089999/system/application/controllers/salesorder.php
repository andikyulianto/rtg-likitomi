...
class SalesOrder extends Controller {
	...
	{	
		...
		$delivery['delivery_date']	= ($this->input->post('delivery_date'))?$this->input->post('delivery_date'):'';
	...
	function inlineedit()
	{
		...
		switch($id)
		{
			case 3: $idname="delivery_date"; $field="Date"; break;
			case 4: $idname="delivery_time"; $field="Time"; break;
		...
		echo 'Today: '.date("G:i").' - <span style="color:blue">'.$field.
			'</span> changed from <span style="color:red">'.$original.
			'</span> to <span style="color:green">'.$newvalue.
			'</span> ';	
	}
