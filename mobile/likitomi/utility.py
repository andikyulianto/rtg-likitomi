...
def currentProcess(machine):
	...
	if(machine=="WH"):
		...
		try:
			...
			item_current = today_plan.filter(actual_wh_start = None).values_list("product_id").order_by('plan_wh_start')
...
def currentTimeProcess(machine):
	...
	if(machine=="CR"):
		try:
			today_plan = FakeStatusTracking.objects.filter(plan_cr_start__year=today.year, plan_cr_start__month=today.month, plan_cr_start__day=today.day).order_by('plan_cr_start').values_list("plan_cr_start","actual_cr_end")
			item_current = today_plan.filter(actual_cr_end = None).values_list("plan_cr_start")[0][0]
		except IndexError, error:
			item_current = 'idle'
	## top current for CV 
	if(machine=="CV"):
