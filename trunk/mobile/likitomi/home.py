...
def section(request):
	...
	if(employee.task == "CR"):
		#temp_contents = FakeStatusTracking.objects.filter(plan_cr_start__year=2010,plan_cr_start__month=11,plan_cr_start__day=19)
		item_plan = FakeStatusTracking.objects.filter(plan_pt_start__year=2010, plan_pt_start__month=11, plan_pt_start__day=19).values_list("plan_cr_start", "plan_cr_end", "product_id", "actual_cr_start", "actual_cr_end").order_by('plan_cr_start')
	if(employee.task == "CV"):
		item_plan = FakeStatusTracking.objects.filter(plan_cv_start__year=2010, plan_cv_start__month=11, plan_cv_start__day=19).values_list("plan_cv_start", "plan_cv_end", "product_id", "actual_cv_start", "actual_cv_end", "cv_machine", "previous_section").order_by('plan_cv_start')
	if(employee.task == "PT"):
		item_plan = FakeStatusTracking.objects.filter(plan_pt_start__year=2010, plan_pt_start__month=11, plan_pt_start__day=19).values_list("plan_pt_start", "plan_pt_end", "product_id", "actual_pt_start", "actual_pt_end").order_by('plan_pt_start')
	if(employee.task == "WH"):
		item_plan = FakeStatusTracking.objects.filter(plan_wh_start__year=2010, plan_wh_start__month=11, plan_pt_start__day=19).values_list("plan_wh_start", "product_id").order_by('plan_wh_start')
