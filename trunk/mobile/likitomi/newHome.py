...
def showWH(eid,section_title):
	...
	#create items for WH
	wh = str(currentProcess("WH"))[2:8]
	item_plan = FakeStatusTracking.objects.filter(plan_wh_start__year=today.year, plan_wh_start__month=today.month, plan_pt_start__day=today.day).values_list("plan_wh_start", "product_id","actual_wh_start").order_by('plan_wh_start')
	items = list(item_plan)
