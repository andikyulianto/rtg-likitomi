...
<table>
	...
	foreach ($resultCorrugatorClamplift->result() as $key)
	{
		...
		$used_df_lkg = $p_width_inch * 25.4  * $used_df * getGrade($key->DF) / 1000000;
		$used_bl_lkg = $p_width_inch * 25.4  * $used_bl * getGrade($key->BL) / 1000000; 
		$used_cl_lkg = $p_width_inch * 25.4  * $used_cl * getGrade($key->CL) / 1000000; 
		
		$used_bm_lkg = $p_width_inch * 25.4  * $used_bm * getGrade($key->BM) / 1000000; 
		$used_cm_lkg = $p_width_inch * 25.4  * $used_cm * getGrade($key->CM) / 1000000; 
